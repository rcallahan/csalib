...
//    returns the index of the successor of pr in Psi[l,r] using binary search
//            (smallest x in [l,r] such that Psi[x] >= pr)
///////////////////////////////////////////
rank_t csa_psi_succ_naive(CSA *csa, rank_t pr, rank_t l, rank_t r)
{
  i64 m,tmp;
  while (l <= r) {
    m = (l+r) / 2;
    tmp = csa->psi(csa,m);
    if (tmp <= pr) l = m+1; else r = m-1;
  }
  return l-1;
}

///////////////////////////////////////////
//  rank_t csa_psi_pred_naive(CSA *csa, rank_t pl, rank_t l, rank_t r)
//    returns the index of the predecessor of pl in Psi[l,r] using binary search
//            (largest x in [l,r] such that Psi[x] <= pl)
///////////////////////////////////////////
rank_t csa_psi_pred_naive(CSA *csa, rank_t pl, rank_t l, rank_t r)
{
  i64 m,tmp;
...
i64 csa_searchsub(int c, CSA *csa, rank_t *ll, rank_t *rr)
{
  ...
  r = csa->psi_succ(csa,r,l0,r0);
  l = csa->psi_pred(csa,l,l0,r0);
  if (l > r) {
    *ll = 1;  *rr = 0;
    return -1;
  }
...
//  pl= 1; pr = csa->n;
  ...
  for (h = keylen-2; h >= 0; h--) {
    ...
    l0 = l;  r0 = r;
    r = csa->psi_succ(csa,pr,l0,r0);
    l = csa->psi_pred(csa,pl,l0,r0);
    if (l > r) goto end;
    len++;
  }
 end:
...
int csa_child_r(CSA *csa, i64 len, rank_t l, rank_t r, uchar *tail, rank_t *ll, rank_t *rr)
{
  ...
  num = csa_child_r_sub(csa, len, l, r, 0, substr, tail, ll, rr);
